...
import com.lisao.wydc.bean.Dishes;
import com.lisao.wydc.config.Config;
import com.lisao.wydc.httputils.HttpUtils;
...
public class DishesDetaliActivity extends ActivityBase implements
	...
	private Dishes dishes = null;// 传递过来的序列化对象
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		dishes = (Dishes) getIntent().getSerializableExtra("dishes");// 获得传递过来的序列化对象
		super.initLeftBtnAndTitle(dishes.getName());// 设置标题为单品名字
		init();
	}
	...
	private void init() {
		...
		paramas.height = mScreenWidth;// 修改控件的高等于宽的值
	...
	public void onClick(View v) {
		...
		case R.id.dishes_detail_gouwuche:// 设置参数，启动线程访问网络
			...
			new Thread(new MyThread(
					...
					ADD_CART)).start();
			break;
		case R.id.dishes_detail_zhifu:// 设置参数，启动线程访问网络
			dialog.show();
			remark = dishes_remark.getText().toString();
			if (remark.equals("") || remark.isEmpty()) {
				remark = dishes.getName();
